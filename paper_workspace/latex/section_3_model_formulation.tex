...
$\mathcal{S}$ & 服务项目 \{1(助餐),$\dots$,6(紧急救助)\} & -- \\
\midrule
\multicolumn{3}{l}{\textbf{Q1 人口演化参数}}\\
\midrule
$\mathbf{S}_i^{(t)}$ & $i$小区第$t$年末三类老人数列向量 & 人 \\
$\mathbf{A}$ & 增生型状态转移矩阵 ($3\times 3$) & -- \\
$\alpha_{\text{S}\to\text{M}}$ & 自理$\to$半失能年转移概率 & 0.045 \\
$\alpha_{\text{M}\to\text{D}}$ & 半失能$\to$失能年转移概率 & 0.10 \\
$\mu$ & 年死亡率（三类通用） & 0.05 \\
$\beta$ & 年新增自理老人占比（60岁进入） & 0.07 \\
$D_{i,e,s}$ & $i$小区$e$类型老人对服务$s$的月均需求 & 次/月 \\
$M_e^{\max}$ & $e$类型老人月消费上限占收入比 & 20\%/25\%/30\% \\
\midrule
\multicolumn{3}{l}{\textbf{Q2 选址-规模优化变量}}\\
\midrule
$y_{i,k}$ & $i$小区建$k$规模服务站的0-1决策变量 & $\{0,1\}$ \\
$x_{i,j}$ & $j$小区由$i$站服务的0-1分配变量 & $\{0,1\}$ \\
$C_k^{\text{build}}$ & $k$规模建设成本 & 18/32/45 万元 \\
$Q_k^{\max}$ & $k$规模日最大服务人次 & 1000/2000/3000 \\
$d_{i,j}$ & 小区间道路距离矩阵 & m \\
$R_{\max}$ & 服务半径硬约束 & 1000 m \\
$B$ & 建设总预算上限 & 120 万元 \\
$\lambda_c,\lambda_s$ & 覆盖与满意度目标权重 & 5.0, 0.5 \\
\midrule
\multicolumn{3}{l}{\textbf{Q3 定价与补贴变量}}\\
\midrule
$p_{j,s}$ & $j$站服务$s$的定价决策变量 & 元/次 \\
$p_s^{\text{base}}$ & 服务$s$的基准价 & 附件2 \\
$R_j$ & $j$站年总营收 & 万元 \\
$C_j^{\text{op}}$ & $j$站年总运营成本 & 万元 \\
$\text{Sub}_j$ & $j$站年政府补贴额 & 万元 \\
$\pi_j$ & $j$站年利润 & 万元 \\
$\text{PR}_j$ & $j$站利润率 $=\pi_j/(R_j+\text{Sub}_j)$ & \% \\
$\text{PR}_{\max}$ & 监管利润率上限 & 8\% \\
$\varepsilon$ & 词典序营收弱激励系数 & $10^{-4}$ \\
\midrule
\multicolumn{3}{l}{\textbf{满意度函数}}\\
\midrule
...
\midrule
\multicolumn{3}{l}{\textbf{Q4 鲁棒性指标}}\\
\midrule
$\rho$ & 多维度韧性系数 $=1-|\Delta X/X_{\text{baseline}}|$ & $[0,1]$ \\
$\Delta\theta$ & 参数扰动向量（$\beta,\alpha_{\text{S}\to\text{M}},\alpha_{\text{M}\to\text{D}},$ op, $B$） & -- \\
\bottomrule
\end{tabular}
\end{table}
...
为响应多模型交叉验证的方法论原则，本文同时构造马尔可夫链与Leslie矩阵两套独立的人口演化模型，以数值结论的高度一致性作为预测可信度的数理担保。
...
题设动力学受三个独立机制支配：(1) 状态退化——自理$\xrightarrow{0.045}$半失能$\xrightarrow{0.10}$失能，后者为吸收壁；(2) 自然死亡——年均$\mu=0.05$概率退出；(3) 新生注入——每年新增$\beta=0.07$（当年总量）的60岁自理老人。编码为增生型转移矩阵$\mathbf{A}$：
...
Q2是经典\textbf{最大覆盖选址}与\textbf{有容量设施选址}的复合变体：1000m服务半径对应MCLP覆盖逻辑\cite{church1974}，三种规模的硬容量约束对应CFLP多容量等级\cite{daskin1995}，满意度$S$的分段阶梯函数带来了额外的非线性。本文将所建模型命名为\textbf{MILP-MCLP-ES}，通过Big-M线性化将非凸满意度规则转化为混合整数线性不等式组，以分支定界求全局最优。淘汰方案：GA/PSO在非凸可行域中无最优性保证；AHP/TOPSIS以评价替代优化，丧失空间-容量-满意度耦合信息。

\subsubsection{目标函数}

Q2需同时最大化服务覆盖率与加权综合满意度，构成双目标优化。两项的量纲与值域存在本质差异——覆盖率为比率量纲$\in[0,1]$，满意度为序数效用$\in[0.6,1.0]$——直接以$\lambda\in[0,1]$线性组合将导致量纲混淆与权重语义模糊。本文改用\textbf{效用定标加权}：为覆盖小区数指派权重$w_c=5.0$，为满意度之和指派权重$w_s=0.5$，使两项对目标函数的边际贡献处于可比较的数量级：

\begin{equation}
\max \; w_c \cdot \underbrace{\sum_{j\in\mathcal{I}} \min\!\big(1,\sum_{i\in\mathcal{I}}x_{i,j}\big)}_{\text{被覆盖小区数}} \;+\; w_s \cdot \underbrace{\sum_{i\in\mathcal{I}}\sum_{j\in\mathcal{I}} S_{i,j}\,x_{i,j}}_{\text{加权满意度之和}}
\end{equation}

$w_c/w_s=10$的权重比编码了"不落一户"全覆盖的政策偏好：当一小区从"脱网"转为"覆盖"时，目标函数获得$+5.0$的跳跃增量，远大于满意度在$[0.6,1.0]$区间内的梯度变化。此权重比非自由参数，而是Q2.1政策语义的直接数学转译。
...
\textbf{(C4) 唯一分配约束}：每位老人由恰好一个服务站服务——满意度最大化等价于距离最小化，因Q2阶段定价固定$S_3\equiv1.0$。
\begin{equation}
...
\end{equation}
其中30为月均天数，用于将日容量转为月容量，$D_{i,j,e,s}^{\text{actual}}$为Q1.3求得的实际需求。

\textbf{(C6) 站点存在性约束}：
\begin{equation}
x_{i,i} \ge y_{i,1} + y_{i,2} + y_{i,3}, \quad \forall i \in \mathcal{I}
\end{equation}
若在小区$i$建站，该小区老人必须由本站服务——距离为0，满意度最高。
...
Q3在Q2最优选址$\{y_{i,k}^*\}$固化的前提下，允许各站自主定价$p'_{i,s}$，目标最大化满意度，受利润率$\le 8\%$监管约束。定价通过$S_3$影响满意度，此时$x_{i,j}^*$已由Q2固化。

\subsubsection{利润率与补贴约束}

年利润$\Pi_i$含运营收支、政府补贴$\sigma_s^{\text{sub}}=2$元/人次——紧急救助除外——以及20年折旧：
...
\begin{tikzpicture}[
    node distance=0.4cm,
    block/.style={rectangle, rounded corners=3pt, draw=#1, fill=#1!10, line width=1.0pt,
                  minimum width=2.4cm, minimum height=0.85cm, align=center, font=\small\bfseries},
    shared/.style={rectangle, rounded corners=2pt, draw=Slate!50, fill=Slate!8,
                   minimum width=2.2cm, minimum height=0.7cm, align=center, font=\footnotesize},
    arrow/.style={->, >=stealth, line width=1.2pt, color=Slate!60},
    datalabel/.style={font=\scriptsize\itshape, color=Slate},
]
...
% Top: four sub-models
\node[block=NavyBlue] (q1) {Q1 人口预测\\[2pt] {\normalsize Markov + Leslie}};
\node[block=CyanTeal, right=1.0cm of q1] (q2) {Q2 选址-规模\\[2pt] {\normalsize MILP-MCLP-ES}};
\node[block=RoseRed, right=1.0cm of q2] (q3) {Q3 差异化定价\\[2pt] {\normalsize 词典序 MILP}};
\node[block=AmberGold, right=1.0cm of q3] (q4) {Q4 鲁棒性检验\\[2pt] {\normalsize 三场景重求解}};

% Arrows between models
\draw[arrow] (q1.east) -- (q2.west) node[midway, above=8pt, datalabel] {$\mathbf{S}_i^{(5)},D^{\text{actual}}$};
\draw[arrow] (q2.east) -- (q3.west) node[midway, above=8pt, datalabel] {$y_{ik}^*,x_{ij}^*$};
\draw[arrow] (q3.east) -- (q4.west) node[midway, above=8pt, datalabel] {$p_s^*,\pi_j^*$};

% Bottom: shared framework
\node[shared, below=1.2cm of q2] (s1) {Big-M 线性化};
\node[shared, below=1.2cm of q3] (s2) {McCormick 包络};
\node[shared, left=0.3cm of s1] (s0) {CBC 分支定界};
\node[shared, right=0.3cm of s2] (s3) {多起点随机重启};

% Dashed lines from shared to models
\draw[arrow, dashed, Slate!50] (s0.north) -- (s0.north |- q1.south);
\draw[arrow, dashed, Slate!50] (s1.north) -- (s1.north |- q2.south);
\draw[arrow, dashed, Slate!50] (s2.north) -- (s2.north |- q3.south);
\draw[arrow, dashed, Slate!50] (s3.north) -- (s3.north |- q4.south);

% Label
\node[font=\footnotesize\bfseries, color=Slate, above=0.15cm of s0] {统一MILP编译框架};

\end{tikzpicture}%
}
\caption{模型族一致性：四阶段串行依赖链与共享求解框架。Q1输出人口与需求预测，Q2固化为选址-分配方案，Q3在固化选址下优化定价，Q4通过参数扰动复用Q2/Q3求解器检验鲁棒性。Big-M线性化、McCormick包络、CBC分支定界与多起点随机重启构成全文统一的MILP方法论内核。}
